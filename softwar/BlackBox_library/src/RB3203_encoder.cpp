...
uint64_t RB3203_encoder::gpioInputPinSel = 1;
uint8_t RB3203_encoder::nextCounterIndex = 0;

void IRAM_ATTR RB3203_encoder::gpio_isr_handler(void* arg) 
{
...
RB3203_encoder::RB3203_encoder(uint8_t aGpioA, uint8_t aGpioB) 
{
...
int32_t RB3203_encoder::getCount() 
...
int16_t RB3203_encoder::getDiff() 
{
    pcnt_get_counter_value(pcnt_unit_t(counterIndex), &PCNT_internal_count);
    pcnt_counter_clear(pcnt_unit_t(counterIndex));
    PCNT_count += PCNT_internal_count;
...
float RB3203_encoder::getFrequency() 
{
...
void RB3203_encoder::clearCount() 
{
    PCNT_count = 0;
}